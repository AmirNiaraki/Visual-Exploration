...
x3,y3,z3 = [],[],[]
x2,y2,z2 = [],[],[]
path = np.load('.\Backup_variables\path16.npy')
path=path[:-2]
bx = [0,2,3,4]
by = [2,1,3,2]
bz = [-1,-1,-1,-1]
...
manual=np.array([[0,0,0],[0,1,0],[1,1,0],[1,1,1],[2,2,1],[2,2,2],[3,3,2]])
manual2=[[0, 0, 0],
 [1 ,0 ,0],
 [2 ,1 ,0],
 [3 ,2 ,0],
...
ax.set_title('RL Path vs Naive Path')
ax.xaxis.set_ticks(np.arange(0., 4.1, 1.))
ax.yaxis.set_ticks(np.arange(0.,4.1,1.))
ax.plot3D(x3, y3, z3, 'gray', label = 'Naive Path')
ax.plot3D(x2, y2, z2, 'blue', label = 'Optimum path with altitude change')
ax.plot3D(x1, y1, z1, 'red', label = 'RL Path in Planar motion')
