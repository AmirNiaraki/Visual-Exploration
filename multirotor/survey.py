...
class SurveyNavigator:
    ...
    def start(self):
        ...
        path = [airsim.Vector3r(i,i,-290) for i in range (750, 15000, 1500)]
        # distance = 0
        ...
        try:
            #result = self.client.moveOnPathAsync(path, self.velocity, trip_time, airsim.DrivetrainType.ForwardOnly, airsim.YawMode(False,0), self.velocity + (self.velocity/2), 1).join()
            result = self.client.moveOnPathAsync(path, self.velocity).join()
